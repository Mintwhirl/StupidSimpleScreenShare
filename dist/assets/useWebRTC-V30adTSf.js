  ...
  return (
    r.useEffect(() => {
      ...
      return (
        ...
        n.addEventListener('pause', m),
        n.addEventListener('ended', d),
        n.addEventListener('error', C),
        n.addEventListener('waiting', L),
        ...
        () => {
          (n.removeEventListener('loadedmetadata', S),
            ...
            n.removeEventListener('pause', m),
            n.removeEventListener('ended', d),
            n.removeEventListener('error', C),
            n.removeEventListener('waiting', L),
            n.removeEventListener('stalled', w));
        }
      );
    }, [v]),
    M.jsx('video', { ref: v, className: a, ...l })
  );
});
W.displayName = 'VideoPlayer';
var _ = {};
const H = [
...
function J(a = !1) {
  const l = [...H];
  return (a && D.length > 0 && l.push(...D), l);
}
function z(a, l, i, T = null) {
  ...
  const b = r.useCallback(
        ...
        if (!(!a || !l))
          try {
            const t = await fetch('/api/candidate', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', ...(i?.authSecret && { 'x-auth-secret': i.authSecret }) },
              body: JSON.stringify({
                roomId: a,
                role: l,
            ...
            (console.error('Error sending ICE candidate:', t), d(`Failed to send ICE candidate: ${t.message}`));
          }
      },
      [a, l, i]
    ),
      ...
      return (
        ...
        (e.ondatachannel = (t) => {
          ...
          ((j.current = c),
            ...
            (c.onmessage = (o) => {
              console.log('Received data channel message:', o.data);
            }));
        }),
        e
      );
        ...
        if (a)
          try {
            const t = await fetch('/api/offer', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json', ...(i?.authSecret && { 'x-auth-secret': i.authSecret }) },
              body: JSON.stringify({ roomId: a, desc: e }),
            });
            if (!t.ok) throw new Error(`Failed to send offer: ${t.status}`);
          } catch (t) {
            (console.error('Error sending offer:', t), d(`Failed to send offer: ${t.message}`));
          }
      },
      [a, i]
    ),
      ...
      async (e) => {
        if (a)
          try {
            ...
            (console.error('Error sending answer:', t), d(`Failed to send answer: ${t.message}`));
          }
      },
      [a, i]
    ),
    O = r.useCallback(async () => {
      ...
      const c = async () => {
        try {
          const o = await fetch(`/api/offer?roomId=${a}`);
          if (o.ok) {
            ...
            if (y.desc) {
              ...
              const p = f.current;
              if (p) {
                await p.setRemoteDescription(y.desc);
                const F = await p.createAnswer();
                (await p.setLocalDescription(F), await V(F));
              }
            }
          } else
            o.status === 404
              ? (e++, e > 10 && (clearInterval(s.current), (t = 5e3), (s.current = setInterval(c, t))))
              : console.error('Unexpected error polling for offers:', o.status);
        } catch (o) {
          console.error('Error polling for offers:', o);
        }
      };
      s.current = setInterval(c, t);
    }, [a, V]),
    $ = r.useCallback(async () => {
      s.current && clearInterval(s.current);
      let e = 0,
      ...
      const c = async () => {
        try {
          const o = await fetch(`/api/answer?roomId=${a}`);
          if (o.ok) {
            const y = await o.json();
            if (y.desc) {
              (clearInterval(s.current), (s.current = null));
              const p = f.current;
              p && (await p.setRemoteDescription(y.desc));
            }
          } else
            o.status === 404
              ? (e++, e > 10 && (clearInterval(s.current), (t = 5e3), (s.current = setInterval(c, t))))
              : console.error('Unexpected error polling for answers:', o.status);
        } catch (o) {
          console.error('Error polling for answers:', o);
        }
      };
      s.current = setInterval(c, t);
    }, [a]),
    x = r.useCallback(async () => {
      (h.current && clearInterval(h.current),
        (h.current = setInterval(async () => {
          try {
            const e = await fetch(`/api/candidate?roomId=${a}&role=${l}`);
            if (e.ok) {
              const t = await e.json();
              if (t.candidates && t.candidates.length > 0) {
                const c = f.current;
                if (c) for (const o of t.candidates) await c.addIceCandidate(o);
              }
            }
          } catch (e) {
    ...
    N = r.useCallback(async () => {
      ...
      try {
        ...
        const t = R();
        ((f.current = t),
          e.getTracks().forEach((o) => {
            t.addTrack(o, e);
          }));
        const c = await t.createOffer({ offerToReceiveAudio: !0, offerToReceiveVideo: !0 });
        return (await t.setLocalDescription(c), await P(c), $(), e);
      } catch (e) {
        throw (
          console.error('Error starting screen share:', e),
          d(`Failed to start screen sharing: ${e.message}`),
          n('disconnected'),
          e
        );
      }
    ...
    A = r.useCallback(async () => {
      ...
      try {
        ...
        throw (
          ...
          d(`Failed to connect to host: ${e.message}`),
          n('disconnected'),
          e
        );
      }
    ...
    I = r.useCallback(async () => {
      try {
        ...
        (console.error('Error stopping screen share:', e), d(`Failed to stop screen sharing: ${e.message}`));
      }
    }, [u]),
